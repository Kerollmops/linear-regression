...
pub fn retrieve_min_max(file: &str) -> Option<(u32, u32)> {
...
fn main() {
    ...
    let (min, max) = retrieve_min_max(common::TMP_FILE_MIN_MAX.unwrap_or((0_u32, 0_u32));
    let norm_mileage = common::normalize(ask_mileage() as f32, min, max);

    let estimated = common::estimate_price(norm_mileage, theta0, theta1);
